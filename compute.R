...
cat("Default cores set to:", use_ncores)
#---------------------------------------------
pathIn <- list(
...
percMissDF$xo <- totxo[match(percMissDF$Mouse,names(totxo))]
...
percMissDF[,c(1,4,5), drop = F]%>% 
  kable(col.names = c("Mouse","Missing (%)","Crossovers")) %>% 
  ...
  row_spec(which(percMissDF$xo >= xo_thresh), bold = T,
           color = "white",
           background = "Red") %>%
  save_kable(file = "../output/plots/missXO_kable.pdf")
