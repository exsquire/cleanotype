...
saveRDS(percMissDF, file = "../output/objects/percMiss_SampDF.rds")

pc_thresh <- mean(percMissDF$val)+(3*sd(percMissDF$val))
...
percMiss %>% ggexport(filename = "../output/plots/PercMiss_Plot.pdf") 
###################################
...
grid.arrange(dupePlot, tbl, nrow = 2, as.table = T, heights = c(2,1)) %>%
  ggsave(filename = "../output/plots/DupeDiag.pdf", width = 6, height = 4)
...
#removes samples with large +4sd from mean missing or > 20%
#And dropped all null markers
pc_thresh2 <- mean(percMissDF$val)+(4*sd(percMissDF$val))
if(pc_thresh2 < 20){
